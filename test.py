...
class Environment:
    ...
    def run(self):
        self.env = gym.make(ENV, render_mode="human")  # 修改环境为实时渲染模式
        ...
        for episode in range(NUM_EPISODES):            
            ...
            for step in range(MAX_STEPS):
                ...
                action = self.agent.get_action(state, episode)                # 通过动作a_t得到s_{t+1}和done标志
                observation_next, _, terminated, truncated, _ = self.env.step(action.item())
                done = terminated or truncated  # 合并两种结束状态
